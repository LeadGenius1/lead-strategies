# 🌐 Connecting clientcontact.io Domain (Squarespace) to Vercel

## 📋 Overview

This guide shows how to connect your `clientcontact.io` domain that's currently hosted by Squarespace to your ClientContact.IO platform deployed on Vercel.
...
**Current Setup:**
- Domain: `clientcontact.io` (registered/managed in Squarespace)
- Platform: ClientContact.IO (Tier 3) deployed on Vercel
- Need: Point `clientcontact.io` to Vercel deployment
...
1. **Deploy ClientContact.IO to Vercel:**
   ```bash
...
1. **Go to Vercel Dashboard:**
   ...
   - Select your ClientContact.IO project
...
3. **Subdomain Handling:**
   - `www.clientcontact.io` → CNAME to Vercel
   - `clientcontact.io` (root) → A record to Vercel IP
...
- [ ] Deploy ClientContact.IO to Vercel
- [ ] Add `clientcontact.io` domain in Vercel dashboard
...
- [ ] Test `clientcontact.io` in browser
...
1. **Use subdomain instead:**
   - `app.clientcontact.io` → Point to Vercel
   - Keep `clientcontact.io` on Squarespace

2. **Configure in Vercel:**
   - Add `app.clientcontact.io` as custom domain
...
- ✅ `www.clientcontact.io` → Points to Vercel deployment (optional)
- ✅ HTTPS/SSL → Automatically enabled by Vercel
- ✅ ClientContact.IO platform → Accessible via custom domain

---

## 🎯 ClientContact.IO Specific Notes

**Platform Details:**
- **Tier:** Tier 3 (Multi-Channel Outreach Platform)
- **Features:** Multi-channel communication, unified inbox, 500 leads
