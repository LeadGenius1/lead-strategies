...
export default function LeadDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (!loading && !user) {
      router.push(`/login?redirect=/dashboard/leads/${params.id}`);
    } else if (user) {
      fetchLead();
    }
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-[#030303]">
      ...
      <section className="relative pt-40 pb-20 md:pt-52 md:pb-32 z-10">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="mb-8">
            <Link href="/dashboard/leads" className="text-purple-400 hover:text-purple-300 font-geist text-sm uppercase tracking-widest mb-4 inline-block">
              ← Back to Leads
            </Link>
            <h1 className="text-5xl md:text-7xl uppercase mb-4 text-white tracking-tighter font-space-grotesk font-light">
              {lead.firstName} <span className="text-gradient">{lead.lastName}</span>
            </h1>
          </div>
          ...
          <div className="bg-[#050505] border border-subtle p-8 space-y-6">
            ...
            <div className="flex gap-4 pt-6 border-t border-subtle">
              <button
                onClick={handleSave}
                disabled={saving}
              ...
              <button
                ...
                className="bg-transparent border border-red-500/30 text-red-400 px-8 py-3 text-sm font-bold tracking-widest uppercase hover:bg-red-500/10 transition-colors font-geist"
              >
                Delete Lead
              </button>
            </div>
