...
async function generateAIResponse(context, conversationHistory) {
  ...
  if (!ANTHROPIC_API_KEY) {
    return null; // Fall back to template if AI not configured
  }
  ...
  try {
    ...
    const systemPrompt = `You are a helpful customer service assistant. Generate a professional, friendly, and contextual response based on the conversation history and context provided. Keep responses concise but helpful.`;
