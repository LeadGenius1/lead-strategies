...
// Starts Email Infrastructure Sentinel (8th Agent) when REDIS_URL is set and Redis is reachable
export async function register() {
  if (process.env.NEXT_RUNTIME !== 'nodejs' || !process.env.REDIS_URL) return;
  try {
    const { canConnectToRedis } = await import('@/lib/redis');
