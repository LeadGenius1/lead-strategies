...
const API_URL = process.env.NEXT_PUBLIC_API_URL || process.env.RAILWAY_API_URL || 'https://backend-wheat-beta-15.vercel.app'
...
// ==================== TACKLE.IO APIs (Tier 5 - Enterprise CRM) ====================

// Tackle Dashboard API
...
export const dealsAPI = {
  ...
  updateStage: async (id, stage) => {
    const response = await api.patch(`/tackle/deals/${id}/stage`, { stage })
    return response.data
  },
}
...
export const activitiesAPI = {
  ...
  complete: async (id) => {
    const response = await api.patch(`/tackle/activities/${id}/complete`)
    return response.data
  },
  getUpcoming: async () => {
...
// Calls API (Voice calling with Twilio)
export const callsAPI = {
  ...
  initiate: async (contactId, phoneNumber) => {
    const response = await api.post('/tackle/calls/initiate', { contactId, phoneNumber })
    return response.data
  },
  end: async (callId) => {
    const response = await api.post(`/tackle/calls/${callId}/end`)
    return response.data
  },
}
...
// Sequences API (Automated outreach)
export const sequencesAPI = {
  getAll: async () => {
    const response = await api.get('/tackle/sequences')
    return response.data
  },
  getById: async (id) => {
  ...
  enroll: async (sequenceId, contactIds) => {
    const response = await api.post(`/tackle/sequences/${sequenceId}/enroll`, { contactIds })
    return response.data
  },
}
...
export const teamsAPI = {
  ...
  addMember: async (teamId, userId, role) => {
    const response = await api.post(`/tackle/teams/${teamId}/members`, { userId, role })
    return response.data
  },
  removeMember: async (teamId, userId) => {
    const response = await api.delete(`/tackle/teams/${teamId}/members/${userId}`)
    return response.data
  },
}

// Tackle Analytics API
export const tackleAnalyticsAPI = {
