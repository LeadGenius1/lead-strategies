...
export function useSchedulerFeed() {
  ...
  const handleEvent = useCallback((event) => {
    switch (event.type) {
      ...
      // ── Execution events ─────────────────────────────────────────
      case 'exec_start':
        updateFeedItemByOutputId(event.outputId, { executionStatus: 'executing', execId: event.execId });
        pushFeedItem({ ...feedItemFromEvent(event), cardType: CARD_TYPE.STATUS });
        break;

      case 'exec_complete':
        updateFeedItemByOutputId(event.outputId, { executionStatus: 'completed', execId: event.execId, execResult: event.result });
        pushFeedItem({ ...feedItemFromEvent(event), cardType: CARD_TYPE.RESULT });
        break;

      case 'exec_failed':
        updateFeedItemByOutputId(event.outputId, { executionStatus: 'failed', execId: event.execId, execError: event.error });
        pushFeedItem({ ...feedItemFromEvent(event), cardType: CARD_TYPE.ALERT });
        break;
  ...
  // ── Update existing feed items by outputId (for execution status) ──
  function updateFeedItemByOutputId(outputId, updates) {
    if (!outputId) return;
    feedRef.current = feedRef.current.map((item) =>
      item.outputKey === outputId ? { ...item, ...updates } : item
    );
    setFeedItems([...feedRef.current]);
  }
  ...
  const retryExecution = useCallback(async (execId) => {
    try {
      await apiRetryExecution(execId);
    } catch (err) {
      setError(err.response?.data?.error || err.message);
    }
  }, []);
